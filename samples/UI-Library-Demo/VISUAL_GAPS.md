...
| Dashed/per-side stroke | Public `BorderWidth` is still a scalar solid stroke and has no dash pattern. `InnerShadow`/`InnerGlow` now use the shared typed `EffectSet` on visual rectangles and text. | Empty-state boxes use solid strokes; the sample exercises inner shadow on a border and inner glow on caption text. |
