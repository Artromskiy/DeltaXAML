...
| Dashed stroke | Public borders support uniform and per-side solid widths; dash patterns are not part of the current stroke contract. `InnerShadow`/`InnerGlow` now use the shared typed `EffectSet` on visual rectangles and text. | Empty-state boxes use solid strokes; the sample exercises inner shadow on a border and inner glow on caption text. |
| Independent border sides | `BorderThickness` accepts one, two or four values and reaches the rounded and solid stroke shader paths. | Toolbar uses `BorderThickness="0,1,0,1"`; focused loader and generated-path checks cover uniform, horizontal/vertical and left/top/right/bottom widths. |
