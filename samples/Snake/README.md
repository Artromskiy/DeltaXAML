...
For normal project builds, consume the generated producer program/factory API
from the `DeltaShader.Tool` NuGet integration for `ShaderArtifact`, `ShaderAbi`
and typed packers. The `.shader.json` files are compiler diagnostics only; do
