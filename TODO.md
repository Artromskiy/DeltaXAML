...
- [x] Add the primitive virtualizing repeater: source view, realization store,
...
- [x] Compile `Self`, template owner, named-element, cached ancestor,
  multi-source/function and explicitly cultured formatting bindings.
- [x] Add generated typed attached-property slots and use them for grid/layout
  metadata without a global object-keyed store.
- [x] Compile property/data/multi conditions and visual states into dependency
...
- [x] Add descriptor-bound behavior capabilities and one allocation-bounded
  gesture arena covering tap, multiple tap, long press, drag/pan, swipe and
  pinch with host-provided timestamps.
- [x] Add the common overlay/focus-scope primitive, then implement slider,
...
- [x] Add rich paragraph runs, paragraph-level bidi/wrapping/shaping, inline
  hit ranges and hyperlink semantic activation while preserving the frozen
  `UiDisplayList` boundary.
- [x] Add compact brush/image user values, gradient/image resource handling,
  intrinsic metadata, placeholder/error state and precise invalidation without
  renderer ownership.
- [x] Add a borrowed accessibility semantic snapshot and explicit
  localization/culture/direction inputs without platform automation objects in
  DeltaXAML.
- [x] Define narrow host-service requests for clipboard, drag/drop, navigation,
...
- [x] Port every retained MAUI/Avalonia/WinUI/Uno-inspired sample to the
...
- [x] Extend the architecture gate for virtualization, relation caches,
