...
public static class XamlLoader
{
    ...
    private static UiElement? Read(XmlReader r, List<XamlDiagnostic> d, XamlTypeRegistry? registry) { var line = (r as IXmlLineInfo)?.LineNumber ?? 0; UiElement? e = r.LocalName switch { "Panel" => new Panel(), "StackPanel" => new StackPanel(), "Border" => new Border(), "Grid" => new Grid(), "ContentControl" => new ContentControl(), "Button" => new Button(), "ToggleButton" => new ToggleButton(), "TextBlock" => new TextBlock(), "TextBox" => new TextBox(), "NumericEditor" => new NumericEditor(), "ScrollViewer" => new ScrollViewer(), "ComponentInspector" => new ComponentInspector(), "EditorShell" => new EditorShell(), _ => null }; if (e is null && registry is not null) registry.TryCreate(r.LocalName, out e); if (e is null) { d.Add(new("XAML002", $"Unsupported element '{r.LocalName}'.", line, 1)); if (!r.IsEmptyElement) r.Skip(); return null; } while (r.MoveToNextAttribute()) Apply(e, r.LocalName, r.Value, line, d); r.MoveToElement(); if (r.IsEmptyElement) { r.Read(); return e; } r.Read(); while (!r.EOF && r.NodeType != XmlNodeType.EndElement) { if (r.NodeType == XmlNodeType.Element) { var child = Read(r, d, registry); if (child is not null && e is IUiPanel p) p.Add(child); else if (child is not null && e is ContentControl c) c.Content = child; } else r.Read(); } if (!r.EOF) r.Read(); return e; }
