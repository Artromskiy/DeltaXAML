...
public sealed class UiFrame : IUiFrame
{
    ...
    public void ApplyMutations()
    {
        ...
        for (var i = 0; i < _mutations.Count; i++)
        {
            ...
            if (Find(Root, mutation.Target.Element) is UiElement element && element.TrySet(mutation.Target, mutation.Value, mutation.Invalidation, out _)) AppliedMutationCount++;
            else RejectedMutationCount++;
        }
        _mutations.Clear();
    }
...
public sealed class UiInputRouter : IUiInputRouter, IUiInputDispatcher
{
    ...
    public void Dispatch(in UiInputPacket packet)
    {
        switch (packet.Kind)
        {
            case UiInputPacketKind.Pointer: { var pointer = packet.Pointer; RoutePointer(in pointer); break; }
    ...
    public void RoutePointer(in UiPointerEvent input)
    {
        ...
        if (input.Kind == UiPointerEventKind.Move) _hovered = target;
        ...
        if (input.Kind == UiPointerEventKind.Up) _captured = null;
    }
    public void RouteKey(in UiKeyEvent input)
    {
        ...
        if (_focused is TextBox text) text.ApplyKey(input);
    }
    public void RouteText(in UiTextInput input) { PruneDetachedState(); if (_focused is TextBox text) text.ApplyText(input); }
    public void RouteIme(in UiImeComposition input) { PruneDetachedState(); if (input.IsCommitted) RouteText(new UiTextInput(input.Text)); }
    private void PruneDetachedState()
    {
        if (_focused is not null && Find(_frame.Root, _focused.Id) is null) _focused = null;
        if (_captured is not null && Find(_frame.Root, _captured.Id) is null) _captured = null;
        if (_hovered is not null && Find(_frame.Root, _hovered.Id) is null) _hovered = null;
    }
    ...
    {
        if (element is UiElement e && e.Focusable) result.Add(e);
        foreach (var child in element.Children) CollectFocusable(child, result);
    }
    private static void Raise(UiElement target, in UiRoutedEvent e)
    {
        var path = new List<UiElement>();
        for (UiElement? node = target; node is not null; node = node.Parent as UiElement) path.Add(node);
        if (e.Phase == UiRoutedEventPhase.Preview)
        {
            for (var i = path.Count - 1; i >= 0; i--) if (path[i] is IUiRoutedEventSink preview) preview.OnRoutedEvent(e);
        }
        else
        {
            foreach (var node in path) if (node is IUiRoutedEventSink bubble) bubble.OnRoutedEvent(e);
        }
    }
